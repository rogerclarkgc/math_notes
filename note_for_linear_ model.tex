...
\end{tikzpicture}
%image end
\par The $100(1-\alpha)\%$ confidence interval for $\beta_1$ can be obtained from equation(19):
\begin{equation}
...
\par The corresponding $100(1-\alpha)\%$ confidence interval of $\beta_1$ can be obtained from eq(21) as :
\begin{equation}
...
Basically, a decision rule to test $H_0$ can be frame from eq(19) or eq(21) under the condition when $\sigma^2$ is known or unknown.\\
...
A test statistic for testting $H_0: \beta_1 = 0$ can also be formulated using the analysis of variance technique as follows.
...
\end{equation}
The left hand side of eq(28) is called the sum of squares about the mean or corrected sum of squares of $y$ or $SYY$.\\ 
...
\end{equation}
(式(28)表示了SYY的组成，它由残差平方和($RSS$)和回归项与均值之差平方和($SS_{Reg}$))组成)\\
