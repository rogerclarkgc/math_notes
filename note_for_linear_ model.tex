...
\textsc{\Large Roger's Notes}\\[0.5cm]
\HRule \\[0.4cm]
{ \huge \bfseries Notes for Linear Model}\\[0.4cm]
\HRule \\[1.5cm]
...
\begin{equation}
  \begin{split}
    \lim_{T\rightarrow\infty} \textmd{E}(\hat{\sigma^2}) &= \bar{\textmd{E}}(\hat{\sigma^2})\\
                                                       &= \textmd{E}(s^2)\\
                                                       &= \sigma^2
  \end{split}
\end{equation}
...
\subsection{Analysis of Variance}
We have known that for linear model $y=X\beta + \epsilon$, the variable $y$ follow:
\begin{equation}
  y \sim N(X\beta, \sigma^2 I_T)
\end{equation}
and
\begin{equation}
...
Where
\begin{equation}
  SS_{Reg} = b^{'}X^{'}y = b^{'}X^{'}Xb = y^{'}X(X^{'}X)^{-1}X^{'}y
\end{equation}
is the sum of square due to \textbf{regression}
and
\begin{equation}
\begin{split}
  RSS &= (y-\hat{y})^{'}(y-\hat{y})\\
...
\end{split}
\end{equation}
is the sum of square due to \textbf{residuals}\\
We also should know that
\begin{equation}
\begin{split}
  \frac{SS_{Reg}}{\sigma^2} &\sim \chi^2_K \left(\frac{\beta^{'}X^{'}P_X X\beta}{2\sigma^2}\right)\\
  \frac{SYY}{\sigma^2} &\sim \chi^2_K \left(\frac{\beta^{'}X^{'}X\beta}{2\sigma^2}\right)\\
\end{split}
\end{equation}
The mean square due to regression mean square due to error are
\begin{equation}
\begin{split}
...
Then,
\begin{equation}
  \frac{MS_{Reg}}{MSE} \sim F_{K, T-K} \left(\frac{\beta^{'}X^{'}X\beta}{2\sigma^2}\right)
\end{equation}
\subsection{Testing Linear Hypotheses}
We consider the problem of testing a general linear hypothesis
\begin{equation}
  H_0:\ R\beta = r
\end{equation}
with $R$ a $(K-s)\times K$-matrix and $\textmd{rank}(R)=K-s$, against the alternative
\begin{equation}
  H_1:\ R\beta \neq r
\end{equation}
We have to consider the value of $s$ in (3.43) and (3.44).The general linear hypothesis contains two main special case:\\
\textit{Case 1}: $s=0$. The $K\times K$-matrix $R$ is regular by the assumption $\textmd{rank}(X)=K$, and we may express $H_0$ and $H_1$ in the following form:
\begin{equation}
\begin{split}
  H_0:\ \beta &= R^{-1}r = \beta ^* \\
  H_1:\ \beta &\neq \beta^*
\end{split}
\end{equation}
(当$s=0$时，$R$变为一个满秩方阵，可以取逆，所以可以直接演变成对$\beta$的假设检验)\\
\textit{Case 2}:$s > 0$. We choose an $s \times K$-matrix $G$ complementary to $R$ such that the $K\times K$-matrix $\begin{pmatrix}
  G\\R
\end{pmatrix}$ is regular of rank $K$. Let
\begin{displaymath}
  X \begin{pmatrix} G\\R \end{pmatrix} ^ {-1} = \mathop{\tilde{X}}\limits_{T \times K} = \begin{pmatrix}
    \mathop{\tilde{X_1}} \limits_{T \times s} & \mathop{\tilde{X_2}} \limits_{T \times (K - s)}
  \end{pmatrix}
\end{displaymath}
We also define
\begin{displaymath}
  \mathop{\tilde{\beta_1}}\limits_{s \times 1} = G \beta, \qquad \mathop{\tilde{\beta_2}}\limits_{(K-s) \times 1} = R \beta = r
\end{displaymath}
Then we may transform the original model to:
\begin{equation}
  \begin{split}
    y = X\beta + \epsilon &= X \begin{pmatrix} G \\ R \end{pmatrix}^{-1} \begin{pmatrix} G \\ R \end{pmatrix} \beta + \epsilon \\
                          & = \begin{pmatrix} \tilde{X_1} & \tilde{X_2} \end{pmatrix} \begin{pmatrix} \tilde{\beta_1} \\ \tilde{\beta_2} \end{pmatrix} + \epsilon\\
                          & = \tilde{X_1} \tilde{\beta_1} + \tilde{X_2} \tilde{\beta_2} + \epsilon
  \end{split}
\end{equation}
Then, the hypotheses $H_0$ and $H_1$ are thus equivalent to:
\begin{equation}
\begin{split}
  H_0:\ \tilde{\beta_2} &= r; \quad \tilde{\beta_1}\ \textmd{and}\ \sigma^2 > 0 \  \textmd{arbitrary}\\
  H_1:\ \tilde{\beta_2} & \neq r; \quad \tilde{\beta_1}\ \textmd{and}\ \sigma^2 > 0 \  \textmd{arbitrary}
\end{split}
\end{equation}
(当$s>0$时，我们取一个$s \times K$的矩阵$G$和$R$来构造一个满秩的矩阵$\begin{pmatrix}G\\R\end{pmatrix}$这样我们的线性模型中的$\beta$会分散到两个新的$\tilde{\beta_1}$和$\tilde{\beta_2}$中，
我们的假设检验的形式也发生了改变)\\
$\Omega$ stands for the whole parameter space (either $H_0$ or $H_1$ is valid) and $\omega \in \Omega$ stands for subspace in which only $H_0$ is true; thus
\begin{equation}
\begin{split}
  \Omega &= {\beta;\sigma^2\ : \ \beta \in \mathbb{R}^K, \sigma^ > 0 }\\
  \omega &= {\beta;\sigma^2\ : \ \beta \in \mathbb{R}^K \ \textmd{and} \ R\beta = r;  \sigma^2 > 0 }
\end{split}
\end{equation}
(假设检验的参数空间$(\beta, \sigma^2)$是$\Omega$和$\omega$，分别对应整个参数空间以及当零假设成立时的假设空间)\\
Then, the test statistic we will use the likelihood ratio
\begin{equation}
\begin{split}
  \lambda (y) &= \frac{\textmd{max}_{\omega}L(\Theta)}{\textmd{max}_{\Omega}L(\Theta)}\\
              &= \left(\frac{\hat{\sigma^2_{\omega}}}{\hat{\sigma^2_{\Omega}}}\right)^{-\frac{T}{2}}
\end{split}
\end{equation}
where $\hat{\sigma^2_{\omega}}$ and $\hat{\sigma^2_{\Omega}}$ are ML estimators of $\sigma^2$ under $H_0$ and in $\Omega$
\par The random variable $\lambda (y)$ can take values between 0 and 1, if $H_0$ is true, the numerator of $\lambda (y)$ gets closer to the denominator,
so that $\lambda (y)$ should be close to 1 in repeated samples. On the other hand, $\lambda (y)$ should be close to 0 if $H_1$ is true.
\par Consider the linear transform of $\lambda (y)$:
\begin{equation}
\begin{split}
  F &= {(\lambda (y))^{-T/2}}(T-K)(K-s)^{-1}\\
    &= \frac{\hat{\sigma^2_{\omega}} - \hat{\sigma^2_{\Omega}}}{\hat{\sigma^2_{\Omega}}} \cdot \frac{T-K}{K-s}
\end{split}
\end{equation}
If $\lambda \rightarrow 0$, then $F \rightarrow \infty$ and if $\lambda \rightarrow 1$, then $F \rightarrow 0$, so that:
\begin{enumerate}
  \item \textbf{F is closer to 0 if $H_0$ is true}
  \item \textbf{F is sufficient large if $H_1$ is true}
\end{enumerate}
The distribution of $F$ is different when $s>0$ or $s=0$\\
\textsf{Case 1:} $s=0$
\begin{equation}
  F = \frac{(b-\beta^{*})^{'} X^{'} X (b-\beta^{*})}{(y-Xb)^{'}(y-Xb)} \cdot \frac{T-K}{K}
\end{equation}
\textbf{distribution of $F$}
\begin{enumerate}
  \item $F \sim F_{K, T-K}(\sigma^{-2}(\beta-\beta^{*})^{'} X^{'} X (\beta - \beta^*))$ under $H_1$
  \item $F$ is distributed as central $F_{K, T-K}$ under $H_0:\ \beta = \beta^*$
\end{enumerate}
Given a fixed level of significance of $\alpha$\
\begin{enumerate}
\item Accept $H_0$:\quad $0 \leq F \leq F_{K, T-K, 1-\alpha}$
\item Critical region:\quad    $F > F_{K, T-K, 1-\alpha}$
\end{enumerate}
($F$值会随着$s$的取值不同而变化，当$s=0$，$F$符合参数为$F_{K, T-K}$的$F$分布，设定显著性为$\alpha$的情况下，当$F$值落在$1-\alpha$的$F$分位数内，即$(0, F_{K, T-K,1-\alpha})$时，
接受$H_0$，当超出$F_{K, T-K,1-\alpha}$时，拒绝$H_0$)\\
\textsf{Case 2:} $s > 0$
\begin{equation}
  F = \frac{(b_2 - r)^{'}D(b_2 - r)}{(y - Xb)^{'}(y - Xb)}\frac{T - K}{K - s}
\end{equation}
\textbf{distribution of $F$}
\begin{enumerate}
  \item Accept $H_0$:\quad $0 \leq F \leq F_{K-s, T-K, 1-\alpha}$
  \item Critical region:\quad $F > F_{K-s, T-K, 1-\alpha}$
\end{enumerate}
\subsection{Checking the Adequacy of Regression Analysis}
\subsubsection{Univariate Regression and Multiple Regression}
The basic model $y_t = \beta_0 + \beta_1 x_1 + \epsilon_t$ is appropriate when $b_1$ should be significantly different from zero.
\par Formally, we compare the models:
\begin{displaymath}
  \begin{split}
  H_0:& y_t = \beta_0 + \epsilon_t, \\
  H_1:& y_t = \beta_0 + \beta_1 x_t + \epsilon_t,
\end{split}
\end{displaymath}
by comparing testing $H_0: \beta_1 = 0$ against $H_1: \beta_1 \neq 0$, then the likelihood ratio test statistic is given by:
\begin{equation}
  \begin{split}
    F_{1, T-2} = & \frac{b_1^2 SXX}{s^2}\\
               = & \frac{SS_{Reg}}{RSS} (T-2)\\
               = & \frac{MS_{Reg}}{s^2}
  \end{split}
\end{equation}
\par If we consider more than two regressors, then we comparing testing:
\begin{displaymath}
  \begin{split}
    H_0:& y = \textbf{1}\beta_0 + \epsilon \\
    H_1:& y = \textbf{1}\beta_0 + X\beta_* + \epsilon
  \end{split}
\end{displaymath}
the test statistic $F$ is:
\begin{equation}
  F_{K, T-K-1} = \frac{SS_{Reg}/K}{s^2}
\end{equation}
\subsection{Model choice based on an adjusted coefficient of determination}
\par Denote by $X_1, \ldots, X_K$ all available regressors, and let ${X_{i1}, \ldots, X_{ip}}$ be a subset of $p \leq K$ regressors, the coefficient of determination:
\begin{equation}
  R_p^2 = 1 - \frac{RSS_p}{SYY}
\end{equation}
is inappropriate to compare a model with $K$ and one with $p < K$. \textbf{because $R^2_p$ always increases if an additional regressor is incorporated into the model, irrespective of its values}.
The full model always has the greatest value of $R^2$
\begin{Theorem}
  Let $y=X_1\beta_1 + X_2 \beta_2 + \epsilon = X\beta + \epsilon$ be the full model and $y = X_1 \beta + \epsilon$ be a submodel, Then we have
  \begin{equation}
    R_X^2  - R_{X_1}^2 \geq 0.
  \end{equation}
\end{Theorem}
\par On the basis of Theorem 3.10.1 we define the statistic
\begin{equation}
  F-change = \frac{(RSS_{X_1} - RSS_X)/(K - p)}{RSS_X/(T-K)}
\end{equation}
which is distributed as $F_{K-p, T-K}$ under $H_0$: "submodel is valid". In model choice procedures, $F-change$ tests for significance of the change of
$R^2_p$ by adding additional $K-p$ variables to the submodel\\
($F-change$是一个用于检验当引入新的回归因子时，$R^2_p$是否变大的统计量，这个统计量遵循$F_{K-p, T-K}$的分布，当它趋于0时，表示零假设“原来的小模型有效”成立)
\par we also define a changed coefficient of determination adjusted by the degrees of freedom of multiple model:
\begin{equation}
  \bar{R}_p^2 = 1 - \frac{T-1}{T-p}(1 - R_p^2)
\end{equation}
if we consider two models, the smaller of which is supposed to be fully contained in the bigger, and then we have:
\begin{displaymath}
  \bar{R}_{p + q}^2 < \bar{R}_{p}^2
\end{displaymath}
then smaller model obviously shows a better goodness of fit\\
(对于多元模型，当回归因子增多时，标准的$R^2$会随因子增多而增大，并且这个增大于引入的新回归因子的数值无关，因此需要一个调整过的$\bar{R}^2$来帮助我们判断回归的好坏)
\subsection{Identification and Quantification of Multicollinearity}
For basic model $y=X\beta + \epsilon$, when $X$ is deficient in rank, leading to singularity of $X^{'}X$.
\par The spectral decomposition of the symmetric matrix $X^{'}X$ is
\begin{equation}
  X^{'}X = P\Lambda P^{'}
\end{equation}
with $P = (p_1, \ldots, p_K)$ and $PP^{'}=I$, the model(3.59) can be written as
\begin{equation}
  \begin{split}
    y =& XPP^{'}\beta + \epsilon \\
  ...
  \end{split}
\end{equation}
with $\tilde{X} = XP$, $\tilde{\beta} = P^{'}\beta$, and $\tilde{X}^{'} \tilde{X} = P^{'}X^{'}XP = \Lambda = \textmd{diag}(\lambda_1, \ldots, \lambda_K)$,
so that the columns vectors of $\tilde{X}$ are orthogonal. The elements of $\tilde{\beta}$ are called regression parameters of the \textbf{principal components}.
\subsubsection{Principal Components Regression}
We start at model(3.60)
\begin{displaymath}
  y = XPP^{'}\beta + \epsilon = \tilde{X}\tilde{\beta} + \epsilon
\end{displaymath}
Let the columns of the columns of the orthogonal matrix $P=(p_1, \ldots, p_K$ of the eigenvetors of $X^{'}X$ be numbered according to the magnitude of the eigenvalues
$\lambda_1 \geq \lambda_2 \geq \cdots \geq \lambda_K$, Then $\tilde{x_i} = Xp_i$ is the $i^{th}$ principal component and we get
\begin{equation}
  \tilde{x_i}^{'} \tilde{x_i} = p^{'}_i X^{'} X p_i = \lambda_i
\end{equation}
We now assume exact multicollinearity. Hence $\textmd{rank}(X) = K-J$ with $J \geq 1$. We get
\begin{equation}
  \lambda_{K-J+1}=\cdots=\lambda_K=0
\end{equation}
（这里使用的其实是特征值分解的方法，如果$X$存在共线性，那么它的特征值形成的对角阵中对角线上必然有一部分特征值是0，依据这一特点可以构建后续的矩阵，分离共线性部分和独立的部分）\\
According to the subdivision of the eigenvalues into the groups of zeros and non zeros, we define the subdivision:
\begin{displaymath}
  \begin{split}
    P=(P_1, P_2),\quad \Lambda &= \begin{pmatrix}
      \Lambda_1 & 0\\
      0        & 0
    \end{pmatrix}, \quad \tilde{X} = (\tilde{X_1}, \tilde{X_2}) = (XP_1, XP_2)\\
  ...
  \end{split}
\end{displaymath}
Then we know $\tilde{X_2}=0$ according to eq(3.61), we now obtain:
\begin{equation}
  \begin{split}
    y &= \tilde{X_1}\tilde{\beta_1} + \tilde{X_2}\tilde{\beta_2} + \epsilon \\
      &= \tilde{X_1}\tilde{\beta_1} + \epsilon.
  \end{split}
\end{equation}
Then OLS estimate of the $(K-J)$ vector of $\tilde{\beta_1}$ is $b_1 = (\tilde{X}_1^{'} \tilde{X}_1)^{-1} \tilde{X}_1^{'} y$.The 
OLS estimate of the full vector $\tilde{\beta}$ is\
\begin{equation}
  \begin{split}
    \begin{pmatrix}
      b_1\\
