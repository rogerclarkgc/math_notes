...
Y_{ij} = \mu + a_i + b_i + e_{ij} \quad (i=1, \cdots, k;j=1, \cdots, l)
\end{equation}
随机误差$e_{ij}$的同样满足$E(e_{ij}) = 0;\textrm{Var}(e_{ij}) \in (0, \infty)$，而另一部分$\mu + a_i + b_i$是在水平组合$(i, j)$下的平均效应。其中$\mu$是总体平均，是一切水平组合下的平均值，相当于一个基准。$a_i$是因素$A$在水平$i$下的平均效应，越大表明效应越明显，$b_i$的意义与$a_i$类似。我们还要让其满足如下条件：
\begin{equation}
